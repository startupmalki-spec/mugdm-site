...
import { notifyBillApproved } from '@/lib/bookkeeper/bill-notifications'
...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    if (currentStatus !== 'pending' || !canTransition(currentStatus, 'approved')) {
      return NextResponse.json(
        { error: `Cannot approve bill in status '${currentStatus}'` },
        { status: 409 }
      )
    }
    ...
    const { data: updated, error: updErr } = await supabase
      ...
      .eq('id', id)
      .eq('status', 'pending')
      .select('id, status, approved_by, approved_at, bill_number, total, currency')
      .maybeSingle()

    if (updErr || !updated) {
      return NextResponse.json(
        { error: 'Failed to approve bill — status may have changed' },
        { status: 409 }
      )
    }

    void notifyBillApproved({
      billId: updated.id,
    ...
    console.error('[API] bills/approve failed:', error)
