...
const COUNTER_CONFIG: Record<string, { target: number; prefix: string; suffix: string }> = {
  affordable: { target: 90, prefix: "", suffix: "%" },
      ...
      <div className="relative z-[1] max-w-[1280px] mx-auto px-6">
        ...
        <StaggerContainer stagger={0.12} className="grid grid-cols-1 sm:grid-cols-2 gap-6 max-w-[960px] mx-auto">
          {ITEM_KEYS.map((key) => {
            ...
            const counter = COUNTER_CONFIG[key];
            const staticStat = STATIC_STATS[key];
            return (
              <StaggerItem key={key}>
                <TiltCard
                  tiltAmount={5}
                ...
                >
                  ...
                  <div className="relative flex items-start gap-5">
                    <div className="shrink-0 w-14 h-14 rounded-xl bg-primary/10 flex items-center justify-center transition-all duration-400 group-hover:bg-primary/25 group-hover:scale-110 group-hover:rotate-[5deg] group-hover:shadow-[0_0_20px_rgba(30,64,175,0.2)]">
                      <Icon size={26} className="text-primary transition-transform duration-300 group-hover:scale-110" />
                    </div>
                    <div>
                      <div className="flex items-baseline gap-1.5 mb-1">
                        ...
                        <span className="text-[11px] text-muted-foreground uppercase tracking-wider">{t(`${key}.statLabel`)}</span>
                      </div>
                      <h3 className="text-lg font-semibold mb-1.5 tracking-tight">{t(`${key}.title`)}</h3>
